...
static int xmit_datablock(const BYTE *buff,BYTE cmd)
{
	WCHAR t;
	if(wait_ready(100))return 1;//等待准备失效
	xchg_spi(cmd);
	if(cmd!=0XFD)//不是结束指令
	{
		for(t=0;t<512;t++)
			xchg_spi(buff[t]);//提高速度,减少函数传参时间
		xchg_spi(0xFF);//忽略crc
		xchg_spi(0xFF);
		t=xchg_spi(0xFF);//接收响应
...
void init_timer2(void)
{
    ...
    timer_deinit(TIMER2);
    /* ((1+TIM_Prescaler )/鏃堕挓)*(1+TIM_Period ) 
