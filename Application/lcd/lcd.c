...
{  
	u8 i,j;
	u8 *temp,size1;
	if(size==16){temp=Hzk16;}//閫夋嫨瀛楀彿
	if(size==32){temp=Hzk32;}
  LCD_Address_Set(x,y,x+size-1,y+size-1); //璁剧疆涓�涓眽瀛楃殑鍖哄煙
  size1=size*size/8;//涓�涓眽瀛楁墍鍗犵殑瀛楄妭
	temp+=index*size1;//鍐欏叆鐨勮捣濮嬩綅缃�
	for(j=0;j<size1;j++)
	{
		for(i=0;i<8;i++)
		{
		 	if((*temp&(1<<i))!=0)//浠庢暟鎹殑浣庝綅寮�濮嬭
			{
				LCD_WR_DATA(color);//鐐逛寒
			}
			else
			{
				LCD_WR_DATA(BACK_COLOR);//涓嶇偣浜�
			}
		}
		temp++;
	 }
...
{          
	u16 i,j; 
	LCD_Address_Set(xsta,ysta,xend,yend);      //璁剧疆鍏夋爣浣嶇疆
	for(i=ysta;i<=yend;i++)
	{													   	 	
		for(j=xsta;j<=xend;j++)LCD_WR_DATA(color);//璁剧疆鍏夋爣浣嶇疆
	} 					  	    
}
...
void LCD_ShowBmp(unsigned char *data)
{
	int i,j;
